...
export function TemperatureChart({
  ...
  const extractPoint = (state: unknown): TooltipPoint | null => {
    ...
    const point = data.find(
      (item) => item.recordedAtMs === ms && typeof item.temperatureC === 'number',
    );
    if (!point || typeof point.temperatureC !== 'number') return null;
    return { recordedAtMs: point.recordedAtMs, value: point.temperatureC };
  };
        ...
        {activeTooltip && (
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              <span className="inline-block h-2.5 w-2.5 rounded-full" style={{ backgroundColor: chartColors.temperature.line }} />
              <span>Hőmérséklet : {activeTooltip.value.toFixed(1)}°C</span>
            </div>
