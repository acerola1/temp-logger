...
- [x] `hooks/usePhotoUpload.ts` hook létrehozása:
...
- [x] CuttingsPage képfeltöltés átírása `usePhotoUpload` használatára
- [x] SessionEventsDialog képfeltöltés átírása `usePhotoUpload` használatára
- [x] Mobilos kamera/galéria picker logika közösítése (jelenleg mindkét helyen inline)
...
- [x] `CuttingsList.tsx` kiemelése — lista nézet (szűrés, rendezés, kártya megjelenítés)
- [x] `CuttingDetail.tsx` kiemelése — részletes nézet (adatok, fotók, öntözési napló)
- [x] `CuttingForm.tsx` kiemelése — létrehozás és szerkesztés űrlap (react-hook-form)
- [x] `CuttingPhotoGallery.tsx` kiemelése — fotó galéria + feltöltés + törlés + nézegető
- [x] `CuttingsPage.tsx` maradjon vékony orchestrátor: routing, állapot összekötés, hook-ok

**QA:**
- [x] CuttingsPage.tsx 300 sor alatt van
- [x] Minden kiemelt komponens önálló fájlban, importálva a CuttingsPage-ből
- [x] URL navigáció működik: `/cuttings` → lista, `/cuttings/:id` → részletes nézet
- [x] Böngésző vissza gomb helyesen navigál lista ↔ részletes nézet között
- [x] Fotó nézegető (lightbox) nyitás/zárás/navigáció működik

## 5. SessionEventsDialog szétbontása (~604 sor → ~3 fájl)

- [x] `SessionEventForm.tsx` kiemelése — esemény létrehozás/szerkesztés űrlap
- [x] `SessionEventList.tsx` kiemelése — események listája szerkesztés/törlés gombokkal
- [x] `SessionEventsDialog.tsx` maradjon a dialog keret + állapot orchestráció
...
- [x] Kézi `useAsyncAction` hook szükségességének felülvizsgálata — a `useMutation` nagyrészt kiváltja

**QA:**
- [x] `grep -r "useState.*loading\|useState.*saving\|useState.*error" src/components/` — jelentősen kevesebb találat mint előtte
- [x] Oldal újratöltés: adat gyorsan megjelenik (cache)
- [x] Mutation után az érintett lista automatikusan frissül (invalidation)
- [x] Loading állapot megjelenik adatlekérés közben
- [x] Hálózati hiba: hibaüzenet jelenik meg, retry lehetséges
- [x] React Query DevTools-ban láthatók a query-k (dev módban)
